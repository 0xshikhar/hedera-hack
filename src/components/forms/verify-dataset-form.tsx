...
import { useVerifierContract } from "@/hooks/blockchain/useVerifierContract";
...
export function VerifyDatasetForm() {
  ...
  const { getDataset } = useDatasetContract();
  const { verifyDataset } = useVerifierContract();
  ...
  useEffect(() => {
    ...
    loadDatasetDetails();
  }, [datasetIdParam, isConnected, form, getDataset]);
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      // Call the verification contract with the connected wallet address
      await verifyDataset(datasetId, datasetHash, signature, address);
      ...
      <Card className="w-full max-w-lg mx-auto">
        ...
        <CardContent>
          <div className="text-center space-y-4">
            ...
            <div className="mt-6">
              {dataset && (
                <VerificationStatus datasetId={Number(dataset.id)} />
              )}
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{dataset.id.toString()}</p>
                </div>
                <div>
                  <p className="text-sm font-medium">Rows</p>
                  <p className="text-sm text-muted-foreground">{dataset.numRows.toString()}</p>
                </div>
                <div>
